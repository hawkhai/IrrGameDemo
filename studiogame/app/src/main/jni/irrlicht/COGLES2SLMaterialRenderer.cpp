...
namespace irr
{
    namespace video
    {
		const char* const COGLES2SLMaterialRenderer::sDefaultUniformNames[] =
        {
            "uRenderMode",
            "uMvpMatrix",
            "uWorldMatrix",
        ...
        bool COGLES2SLMaterialRenderer::createShader( GLenum shaderType, const char* shader, const char* shaderFile )
        {
            ...
            if ( !status )
            {
				...
				#ifdef  _IRR_COMPILE_WITH_ANDROID_DEVICE_
				__android_log_print(ANDROID_LOG_INFO,"irrlich", "GLSL shader failed to compile");
				#endif
        ...
        bool COGLES2SLMaterialRenderer::linkProgram()
        {
            ...
            if ( !status )
            {
				...
				#ifdef  _IRR_COMPILE_WITH_ANDROID_DEVICE_
				__android_log_print(ANDROID_LOG_INFO,"irrlich","GLSL shader program failed to link");
				#endif
            ...
            if ( maxlen == 0 && num != 0 )
            {
				...
				#ifdef  _IRR_COMPILE_WITH_ANDROID_DEVICE_
				__android_log_print(ANDROID_LOG_INFO,"irrlich","GLSL: failed to retrieve uniform information");
				#endif
            ...
            for ( int i = 0; i < UniformCount; ++i )
            {
                ...
                else
                {
					...
					#ifdef  _IRR_COMPILE_WITH_ANDROID_DEVICE_
					__android_log_print(ANDROID_LOG_INFO,"irrlich","Unable to find uniform %s",  UniformStringTable[i]);
					#endif
        ...
        bool COGLES2SLMaterialRenderer::setPixelShaderConstant( const c8* name, const f32* floats, int count )
        {
			...
			for (i=0; i < num; i++)
			{
				...
				{
					setUniform(i,(void*)floats,count);
					return true;
				}
