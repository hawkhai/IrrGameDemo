		...
		s64 userData)
{
	...
	if (vertexShaderProgramFileName.size() )
	{
		...
		if (!vsfile)
		{
			...
			#ifdef _IRR_COMPILE_WITH_ANDROID_DEVICE_
			__android_log_print(ANDROID_LOG_INFO, "Irrlicht", "Could not open vertex shader program file %s ",vertexShaderProgramFileName.c_str());
			#endif
		}
	}
	...
	if (pixelShaderProgramFileName.size() )
	{
		...
		if (!psfile)
		{
			...
			#ifdef _IRR_COMPILE_WITH_ANDROID_DEVICE_
			__android_log_print(ANDROID_LOG_INFO, "Irrlicht", "Could not open pixel shader program file %s ",pixelShaderProgramFileName.c_str());
			#endif
		}
	}
